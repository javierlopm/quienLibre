...
/**
 * Creditos de esta implementacion a wongk
 *
 * https://github.com/wongk/MultiSelectSpinner/blob/master/src/com/multiselectspinner/widget/MultiSelectSpinner.java
 *
 */
...
public class MultiSelectSpinner extends Spinner implements OnMultiChoiceClickListener {
    ...
    public void llenarDeAmigos(Context c,int anio, String trimestre){

        List<String> amigos;

        ModeloHorario manejador = new ModeloHorario(c);
        SQLiteDatabase db = manejador.getReadableDatabase();

        String[] args     = {trimestre,Integer.toString(anio)};
        String[] columnas = {"nombre"};                         //Vainas de android



        Cursor cAmigos = db.query(
                true, //Distinct
                "horarios", //Tabla
                columnas,
                "trimestre=?     AND " +
